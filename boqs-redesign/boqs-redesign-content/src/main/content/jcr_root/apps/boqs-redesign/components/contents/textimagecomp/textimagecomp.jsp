...
<c:if test="${isEditMode }">
    <%= StringUtil.getLabelAuthorMode("Text Image Component")%>
</c:if>
...
<%
    ...
    if (image.hasContent() || WCMMode.fromRequest(request) == WCMMode.EDIT) {
        ...
        %><div class="text-image"> <div class="thumb hidden-xs" id="<%= divId %>"><%
        %><% image.draw(out); %><%
        ...
        <% String placeholder = (isAuthoringUIModeTouch && !image.hasContent())
                ? Placeholder.getDefaultPlaceholder(slingRequest, component, "", ddClassName)
                        : ""; %>
           <cq:text property="text" tagClass="caption" escapeXml="true" placeholder="<%= placeholder %>"/>
        </div>
